...
export default function SendTransaction({ web3, network, publicAddress, fetchBalance}) {
  ...
  return (
    <div className="container">
          ...
          <input type="text" disabled={disabled} value={nonce} onChange={(e) => setNonce(e.target.value)} className="full-width" placeholder="Nonce" />
          <button onClick={() => generatePayload()}>Sign Message</button>
          {
          txnHash &&
            <div className="info">
